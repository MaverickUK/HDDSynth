...
try:
    while True:
        ...
        if time.ticks_diff(current_time, last_activity_time) > 50:
            if hdd_activity_counter > 0 or hdd_poll_counter > 0 or fdd_activity_counter > 0:
                print(" | ", end="")
            hdd_activity_counter = 0
            hdd_poll_counter = 0
            fdd_activity_counter = 0
        ...
        if sm_ior.rx_fifo() > 0:
            ...
            if MONITOR_HDD:
                # Standard IDE controller ports
                if 0x1F0 <= addr <= 0x1F7:
                    hdd_activity_counter += 1
                elif addr == 0x3F6: # Alternate status port
                     hdd_poll_counter += 1
            ...
            if MONITOR_KEYBOARD:
                # *** CORRECTED LOGIC ***
                # We only monitor I/O reads on port 0x60 (the data port).
                # The status port (0x64) is polled constantly by the OS to check
                # if a key has been pressed, which created the noise. A read
                # from 0x60 is a much stronger signal that an actual key
                # scancode is being processed by the OS.
                if addr == 0x60:
                    print("k", end="")
        ...
        if sm_iow.rx_fifo() > 0:
            ...
            if MONITOR_KEYBOARD:
                # Writes to 0x60 (data, e.g. setting LEDs) and 0x64 (commands)
                # are both significant and don't typically happen in a tight
                # polling loop, so we can monitor both for write activity.
