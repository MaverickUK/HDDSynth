...
class SDCard:
    ...
    def _cmd(self, cmd, arg, crc, read_timeout=None):
        ...
        def _get_resp():
            # Add a small delay to allow the card to respond
            time.sleep_ms(1)
    ...
    def init_card(self):
        ...
        Includes diagnostic prints to show progress.
        """
        ...
        # CMD0: GO_IDLE_STATE
        print("  -> Sending CMD0 (GO_IDLE_STATE)...")
        self.cs.value(0)
        ...
        # CMD8: SEND_IF_COND
        print("  -> Sending CMD8 (SEND_IF_COND)...")
        ...
        print("  -> Sending ACMD41 with HCS...")
        # Send CMD55 first (ACMD prefix)
        max_attempts = 200
        i = 0
        while self._cmd(55, 0, 0x65) != _R1_IDLE_STATE:
            i += 1
            if i > max_attempts:
                raise OSError("SD card failed to respond to CMD55")
            time.sleep_ms(1)
...
if __name__ == '__main__':
    ...
    if diagnose_sd_card_pins():
        # Proceed with mounting only if the diagnostic test passes
        if mount_sd_card():
            play_audio()
