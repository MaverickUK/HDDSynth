...
try:
    while True:
        ...
        if sm_ior.rx_fifo() > 0:
            ...
            # The HDD activity LED is more often tied to the status/command registers
            # and the data register.
            if addr == 0x1F0 or addr == 0x1F7:
                print("h", end="")  # HDD Read
            # The FDD activity LED is often tied to the motor enable signal.
            elif addr == 0x3F5:
                print("f", end="")  # FDD Read (Data)
            elif addr == 0x60 or addr == 0x64:
                print("k", end="")  # Keyboard Read (Command/Data)
        ...
        if sm_iow.rx_fifo() > 0:
            ...
            # Monitoring writes to the Digital Output Register (0x3F2) and reads
            # from the Data Register (0x3F5) is a better indicator of FDD activity.
            elif addr == 0x3F2 or addr == 0x3F5:
