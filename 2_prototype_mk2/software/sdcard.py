...
def test_spi_connection():
    ...
    try:
        ...
        spi = busio.SPI(
            getattr(board, f"GP{settings.SDCARD_SCK_PIN}"),   # SCK
            getattr(board, f"GP{settings.SDCARD_MOSI_PIN}"),  # MOSI
            getattr(board, f"GP{settings.SDCARD_MISO_PIN}")   # MISO
        )
        print("  ✅ SPI bus initialized")
        
        # Initialize chip select pin
        cs = digitalio.DigitalInOut(getattr(board, f"GP{settings.SDCARD_CS_PIN}"))
...
def list_sd_card_contents():
    ...
    try:
        ...
        print(f"  ✅ Changed to {settings.SDCARD_MOUNT_POINT}")
